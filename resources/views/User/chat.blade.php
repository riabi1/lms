        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                messageInput.addEventListener('input', () => {
                    if (!isTyping) {
                        isTyping = true;
                        fetch('{{ route("messages.typing", $selectedConversation->id) }}', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                'Content-Type': 'application/json',
                ...
                function startPolling() {
                    ...
                    pollInterval = setInterval(() => {
                        fetch('{{ route("messages.fetch", $selectedConversation->id) }}?last_message_id=' + lastMessageId, {
                            method: 'GET',
                        ...
                        .then(data => {
                            if (data.status === 'success' && data.messages.length > 0) {
                                data.messages.forEach(message => {
                                    ...
                                    const isCurrentUser = message.sender_id === {{ Auth::id() }} && message.sender_type === 'App\\Models\\User';
                                    ...
                                    updateConversationList(message, data.conversation);
                                });
                        ...
                        fetch('{{ route("messages.typing-status", $selectedConversation->id) }}', {
                            method: 'GET',
                            headers: { 'Accept': 'application/json' },
                        })
                        .then(response => response.json())
