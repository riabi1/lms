    ...
    <div class="container">
        <div class="card shadow-lg border-0 rounded-4">
            <div class="card-header bg-gradient-red text-white py-3 rounded-top-4">
                <h5 class="mb-0"><i class="bx bx-book-add me-2"></i>Edit Course: {{ $course->course_name }}</h5>
            </div>
            <div class="card-body p-4">
                ...
                <form id="myForm" action="{{ route('instructor.courses.update', $course->id) }}" method="POST" class="row g-4" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="course_name" class="form-control shadow-sm @error('course_name') is-invalid @enderror" id="course_name" value="{{ old('course_name', $course->course_name) }}" required>
                            @error('course_name')
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" name="course_title" class="form-control shadow-sm @error('course_title') is-invalid @enderror" id="course_title" value="{{ old('course_title', $course->course_title) }}" required>
                            @error('course_title')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row g-4 mb-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <img id="showImage" src="{{ $course->course_image ? asset('upload/course_images/thumbnail/' . $course->course_image) : asset('upload/no_image.jpg') }}" 
                                 alt="Course" class="rounded-circle shadow-sm p-1 bg-light" style="width: 100px; height: 100px; object-fit: cover;">
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="row add_item">
                                @forelse ($goals as $item)
                                    <div class="whole_extra_item_delete">
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <input type="text" name="CourseGoals[]" class="form-control shadow-sm" value="{{ $item->goal_name }}" placeholder="Enter a goal">
                                            </div>
                                            <div class="col-md-6">
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-3">
                            ...
                            <input type="number" step="0.01" name="selling_price" class="form-control shadow-sm @error('selling_price') is-invalid @enderror" id="selling_price" value="{{ old('selling_price', $course->selling_price) }}" min="0">
                            @error('selling_price')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="number" step="0.01" name="discount_price" class="form-control shadow-sm @error('discount_price') is-invalid @enderror" id="discount_price" value="{{ old('discount_price', $course->discount_price) }}" min="0">
                    ...
                    <div class="row g-4 mb-4">
                        <div class="col-md-12">
                            ...
                            <textarea name="prerequisites" class="form-control shadow-sm @error('prerequisites') is-invalid @enderror" id="prerequisites" placeholder="Enter prerequisites..." rows="3">{{ old('prerequisites', $course->prerequisites) }}</textarea>
                            @error('prerequisites')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        <div class="col-md-12">
                            ...
                            <textarea name="description" class="form-control shadow-sm @error('description') is-invalid @enderror" id="description" rows="5">{{ old('description', $course->description) }}</textarea>
                            @error('description')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12 text-center mt-4">
                        <button type="submit" class="btn btn-primary px-5 py-2 shadow-sm">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
...
<div style="visibility: hidden">
    <div class="whole_extra_item_add" id="whole_extra_item_add">
        <div class="whole_extra_item_delete mb-2">
            <div class="row">
                <div class="col-md-6">
                    <input type="text" name="CourseGoals[]" class="form-control shadow-sm" placeholder="Enter a goal">
                </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // Trigger AJAX on page load to populate subcategories
        if ($('#category_id').val()) {
